...
const AllEventKeys: React.FC<AllEventKeysProps> = ({
  handleInputChange,
  setSelectedDateTime,
  setdata,
  selectedDateTime,
    ...
    <div>
      <div>
        ...
        <Datetime
          ...
          className=" appearance-none shadow border rounded py-3 text-gray-darker px-2"
      ...
      {eventInput
        .filter(
          (event) => event.name !== "eventType" && event.name !== "eventDate"
        ) // Filter out the object with name 'eventType'
        .map((event) => (
